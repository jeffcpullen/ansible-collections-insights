...
try:
    import requests
    # test_version = Version(requests.__version__)
    # str(test_version)
    # # This version is likely not correct. Had to update release package
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def get_patches(
        self, stale, get_system_advisories, get_system_packages, filter_tags
    ):
    ...
    def verify_file(self, path):
        ...
        if super(InventoryModule, self).verify_file(path):
            if path.endswith(("insights.yaml", "insights.yml")):
                ...
                self.display.vvv(
                    'Skipping due to inventory source not ending in "insights.yaml" nor "insights.yml"'
                )
        return valid
    ...
    def parse(self, inventory, loader, path, cache=True):
            ...
            for system in patching_results:
                ...
                for attribute in system["attributes"]:
                    if attribute != "display_name":
                        patching[display_name][attribute] = system["attributes"][
                            attribute
                        ]
            ...
            if get_patching_info:
                if host_name in patching.keys():
                    self.inventory.set_variable(
                        host_name,
                        vars_prefix + "patching",
                        patching[host["display_name"]],
                    )
                else:
                    self.inventory.set_variable(
                        host_name, vars_prefix + "patching", {"enabled": False}
                    )
        ...
        if get_tags:
            ...
            chunk_size = 20
            systems_by_id_list_split = [
                systems_by_id_list[i: i + chunk_size]
                for i in range(0, len(systems_by_id_list), chunk_size)
            ]
        ...
        for uuid in systems_by_id:
            ...
            if get_tags:
                self.inventory.set_variable(
                    host_name, vars_prefix + "tags", self.parse_tags(system_tags[uuid])
                )
